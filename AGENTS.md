...
- **Library helpers** at `src/lib/library/`:
  ...
  - `src/lib/library/review.ts` — `parseReviewFilters` (incl. deck params `missing`/`shelf`/`proposal`/`isbn`/`shuffle`), `withReviewShelfDefault` (away-from-shelf default), `SHELF_CHECK_MARKER`, `ensureShelfMarkerNote` / `SHELF_DEFER_LINE`, slice/deck query filters for `/library/review` ([067](docs/decisions/067-library-review-sprint-decks.md), [074](docs/decisions/074-review-tab-corrections.md)).
