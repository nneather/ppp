...
- **Library helpers** at `src/lib/library/`:
  ...
  - `src/lib/library/open-library-prefill.ts` — `normalizeIsbnDigits`, `fetchOpenLibraryPrefill` (edition + optional OL work + up to 5 author JSON fetches; `publisher_location`, `edition`, `genreSuggested`), `LIBRARY_OL_PREFILL_KEY` for `/library/add` → `/library/books/new` prefill.
