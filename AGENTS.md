...
- **Projects helpers** at `src/lib/projects/`:
  - `src/lib/types/projects.ts` — `LIFECYCLE_STATUSES`, `HEALTH_STATUSES`, view-models (`ProjectNode`, `WeeklyDraftRow`, etc.).
  - `src/lib/projects/week.ts` — civil **Chicago Sunday** week start (`currentSundayChicago`, `sundayContaining`, `previousSunday`); distinct from invoicing Monday weeks.
  - `src/lib/projects/server/loaders.ts` — `loadProjectTree`, `loadWeekUpdates`, `loadCarryForward`, `collectDescendantIds`, `buildProjectTree`.
  - `src/lib/projects/server/actions.ts` — `saveWeeklyCheckinAction` (PK upsert — footgun NEW-D), `createProjectAction`, `updateProjectAction`, `softDeleteProjectAction`, `undoSoftDeleteProjectAction`.
  - `/projects` — inline tree (`project-tree.svelte`) + metadata sheet (`project-form-sheet.svelte`); `depends('app:projects:tree')`.
