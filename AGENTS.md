...
- **PWA service worker** — [src/service-worker.ts](src/service-worker.ts) (Workbox `injectManifest` via `@vite-pwa/sveltekit`): precache hashed client assets; NetworkFirst navigation HTML with auth/login/`?/` bypass; gated refresh via `<PwaReloadToast />`. Direct devDependency `workbox-window`; Vite 7 inject guard in [src/lib/vite/patch-sveltekit-pwa.ts](src/lib/vite/patch-sveltekit-pwa.ts). Cache-bust + owner verification checklist: [docs/decisions/024-service-worker.md](docs/decisions/024-service-worker.md).
