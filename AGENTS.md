...
- **Invoicing helpers** at `src/lib/invoicing/` (loaders/actions live inline in routes — see Module structure above):
  - `src/lib/invoicing/chicago-date.ts` — `ymdInChicago()`, civil-date helpers (`utcNoonFromYmd`, week/month spans, `calendarMonthContainingYmd()`, `firstOfYearThroughYmd()`), Chicago `Intl` display formatters. Use for server “today,” `/invoicing` surfaces, generate-invoice defaults, and `<TimeEntrySheet>` default date — not `toISOString().slice(0,10)` or host-local `getFullYear()` for user-facing calendar semantics.
  ...
  - `src/lib/invoicing/analytics.ts` — week/month bucket series + range presets (YTD default) + URL helpers for `/invoicing/analytics` ([197](docs/decisions/197-invoicing-analytics.md), [201](docs/decisions/201-invoicing-analytics-range-one-offs.md)); one-offs count as money not hours; unit tests `analytics.test.ts`.
  - `src/lib/invoicing/one-off.ts` — amount parse + ledger payload + generate JSON parse for `is_one_off` charges ([201](docs/decisions/201-invoicing-analytics-range-one-offs.md)); unit tests `one-off.test.ts`.
  - `src/lib/invoicing/mark-paid.ts` — `sent ↔ paid` toggle + UUID parse for list/detail mark-paid ([202](docs/decisions/202-invoicing-list-mark-paid.md)); unit tests `mark-paid.test.ts`.
...
- **PWA service worker** — [src/service-worker.ts](src/service-worker.ts) (Workbox `injectManifest` via `@vite-pwa/sveltekit`): precache hashed client assets + `offline.html` only (no authenticated HTML navigate cache — see [024](docs/decisions/024-service-worker.md)); `setCatchHandler` serves offline shell on failed navigations; SWR for library vocab JSON via [`vocab-cache-paths.ts`](src/lib/library/vocab-cache-paths.ts); gated refresh via `<PwaReloadToast />` (hourly + on-resume `registration.update()`; **auto-apply waiting update on resume when idle** — [082](docs/decisions/082-pwa-update-auto-recover.md), hardened to apply `registration.waiting` immediately + `pageshow`/`focus` fallbacks — [111](docs/decisions/111-pwa-resume-auto-apply-harden.md); **defers silent reload while mid-edit** via [`user-busy.ts`](src/lib/pwa/user-busy.ts) (`isUserBusy`) on desktop tab return + mobile PWA — [186](docs/decisions/186-pwa-defer-reload-while-editing.md); mid-session toast kept, **top-anchored** — [100](docs/decisions/100-pwa-update-banner-nonblocking.md)). Build stamp `__APP_BUILD__` on `/settings`. Direct devDependency `workbox-window`; Vite 7 inject guard in [src/lib/vite/patch-sveltekit-pwa.ts](src/lib/vite/patch-sveltekit-pwa.ts). Light-theme chrome aligned [057](docs/decisions/057-pwa-consistency.md). **Cold-start resilience** ([072](docs/decisions/072-pwa-cold-start-resilience.md), [082](docs/decisions/082-pwa-update-auto-recover.md), [100](docs/decisions/100-pwa-update-banner-nonblocking.md), [186](docs/decisions/186-pwa-defer-reload-while-editing.md), [187](docs/decisions/187-pwa-silent-recovery-harden.md)): inline boot shell in [`app.html`](src/app.html) (`[data-ppp-boot]` removed on layout mount); [`hooks.client.ts`](src/hooks.client.ts) + [`client-recovery.ts`](src/lib/pwa/client-recovery.ts) auto clear-cache+reload on chunk-load failure when idle (unregister all SWs + wipe caches + `location.replace` with `_ppp_recover`; up to **2** silent attempts; settle clears cooldown; fallback banner at **top** only after max silent fails **or** `isUserBusy()`); 12s nav watchdog in root layout.
...
- **Library helpers** at `src/lib/library/`:
  - `src/lib/types/library.ts` — closed enums (`GENRES`, `LANGUAGES`, `READING_STATUSES`, `AUTHOR_ROLES`) + view-models (`BookListRow`, `BookDetail` incl. `copyCount` / **`owned`** ([103](docs/decisions/103-library-not-owned-session-1.md)), `PersonRow`, `ScriptureRefRow`, etc.). Reuse before improvising. Genre taxonomy includes Politics and Policy, Leadership, Literary Criticism, Church Fathers, Ancient Biblical Sources, Children's and Young Adult ([087](docs/decisions/087-library-review-queue-research-cleanup.md)); keep `GENRES` in sync with `books_genre_check`. `books.copy_count` (1–99, default 1) for multi-copy ownership without duplicate rows. `books.owned` (default true) for research stubs.
  ...
  - `src/lib/library/server/loaders.ts` — `loadBookListFiltered` (server-paginated list + `{ filteredCount }`; default **owned-only** unless `includeUnowned` — [103](docs/decisions/103-library-not-owned-session-1.md)), `loadEssaySearchHits` (keyword `q` → essay title / essay-author hits, cap 20; parent-book owned filter — [086](docs/decisions/086-essay-visibility-and-search-lanes.md), [103](docs/decisions/103-library-not-owned-session-1.md)), `loadBookDetail`, `loadSeriesSiblingBooks` / `loadAlsoByAuthorBooks` (book-detail densify — [156](docs/decisions/156-book-detail-densify.md)), `loadSeries`, `loadPublishers`, `loadPublisherBookCounts`, `loadPeople`, `loadPersonBookCounts`, `loadBibleBookNames`, `loadScriptureRefsForBook` (signed URLs via `signStorageUrlsLimited`), `loadScriptureRefsNeedingReview`, `loadEssaysForBook` (essay + `essay_authors` → `EssayRow`), `personDisplayShort/Long`, `loadBookFormPageData`, `countLiveBooks` / `countBooksNeedingReview` (owned-only by default). Used by `/library` and `/library/books/[id]`. Book selects embed publishers via `PUBLISHER_EMBED` (`publishers!books_publisher_id_fkey`) — required after `reprint_publisher_id` FK; unqualified `publishers (...)` returns PGRST201 and loaders silently empty (see [037](docs/decisions/037-publishers-embed-disambiguation.md)).
  ...
  - `src/lib/library/isbn.ts` — `normalizeIsbnDigits`, `parseIsbnWithChecksum`, `isbn10ToIsbn13`, `isbnMatchKeys` (ISBN-10/13 check digit + Goodreads match twins); used by `/library/add`, `fetchOpenLibraryPrefill`, Goodreads import.
  - `src/lib/library/search-tsquery.ts` — `toLibrarySearchTsQuery` (prefix `to_tsquery` for `/library` keyword `q`; partial last names / titles — [140](docs/decisions/140-library-search-prefix-tsquery.md)).
  ...
  - `src/lib/library/publisher-location.ts` — `normalizePublisherLocationTurabian` / `normalizePublisherLocationOrNull` / `resolveUsStatePostal` (US `City, ST` postal only — traditional `Mich.` → `MI`; well-known cities alone; strip `USA`; OL prefill + proposals + loaders + book/publisher save + Turabian facts + copy row — [115](docs/decisions/115-publisher-location-postal.md)).
  - `src/lib/library/citation-abbreviation.ts` — `effectiveSeriesAbbreviation` / `normalizeCitationAbbreviationOrNull` (per-book `citation_abbreviation` overrides `series.abbreviation` for Turabian article footnotes; IVP Black Dictionaries share one series — [125](docs/decisions/125-books-citation-abbreviation.md)).
  ...
  - `src/lib/library/book-detail-related.ts` — `primaryAuthorPersonIds`, `siblingBookLabel`, `compareSiblingBooks` for book-detail densify strips ([156](docs/decisions/156-book-detail-densify.md)); unit tests `__tests__/book-detail-related.test.ts`.
 ...
 - `src/lib/library/server/proposal-actions.ts` — `resolveProposalAction` + `markProposalResolved` + `markProposalPending` (undo) for `book_metadata_proposals` (AI research pass [068](docs/decisions/068-library-review-ai-research-pass.md)); accept path rides `reviewSaveAction` (`proposal_id` + `proposal_resolution` FormData fields). Proposals never clear `needs_review` by themselves.
  ...
  - `src/lib/library/server/url-params.ts` — `parseBookListFilters` / `bookListFiltersToSearchParams` / `libraryListFormAction` (URL is source of truth for `/library` filters; `include_unowned` → `includeUnowned` — [103](docs/decisions/103-library-not-owned-session-1.md); `bible_book` coverage facet — [123](docs/decisions/123-library-bible-coverage-filter.md); list POST actions must keep `?q=` — [209](docs/decisions/209-library-list-search-survives-status.md)).
 ...
 - `src/lib/library/missing-important.ts` — `IMPORTANT_FIELDS` / `computeMissingImportant` / `incompleteCitationCaption` (client-safe; re-exported from `book-actions` for save-time auto-flag) ([094](docs/decisions/094-library-writing-session-gaps.md)).
 ...
 - `src/lib/library/turabian/` — pure-function Turabian 9th-ed. formatters (`formatFootnote` + `shortForm` `ibid`/`short`, `formatBibliography` + optional `{ citeSet: true }` → `N vols.` when `total_volumes > 1` — [157](docs/decisions/157-multivol-systematics-credits-set-bib.md), `formatCompiledBibliography`, `formatEssayFootnote` / `formatEssayBibliography` — unsigned `s.v.`, signed dictionary, TDNT abbreviated, chapter-in-edited-volume dispatch, **essay short form** `Last, "Title," page.`; essay **note/short default `[page]`** when page input empty — bib still uses stored range — [154](docs/decisions/154-essay-note-page-placeholder.md); unsigned lexicon volume cites `BDAG, s.v. "[lemma],"` ([163](docs/decisions/163-bdag-sv-essay-bib-greek-nfc.md)); essay bib `vol:page` for reference works ([163](docs/decisions/163-bdag-sv-essay-bib-greek-nfc.md)); `normalizeCitationText` NFC ([163](docs/decisions/163-bdag-sv-essay-bib-greek-nfc.md)); `copyCitationToClipboard`, `CITATION_CRITICAL_GENRES`, `review-progress.ts` localStorage burndown). QA fixtures: [docs/library-turabian-fixtures.md](docs/library-turabian-fixtures.md) + `WAVE2_FIXTURES` in `__tests__/fixtures.ts`. Structured names via `BookAuthorAssignment` + `parseAuthorAssignment`. `joinNoteSegments` / `joinBibSegments` per Covenant §17.1; `work_type` on `BookCitationInput`. Powers book-detail Copy Footnote/**Short form**/Bibliography (+ page input + **Cite set** checkbox), `/library/bibliography`, and Turabian-first `/library/review` cards. Unit tests: `npm run test`. **`.docx` export ([063](docs/decisions/063-library-wave2-session4-docx-export.md)):** `formatBibliographyEntries` (sorted non-empty entries, shared with clipboard compile) + `html-segments.ts` (`parseCitationHtmlSegments` — `CitationFormatted.html` → italic/plain runs; client-safe) feed `src/lib/library/server/bibliography-docx.ts` (`buildBibliographyDocx`, `docx` package — **server-only, not exported from the turabian barrel** so `docx` stays out of client bundles) behind `GET /library/bibliography/download?ids=` (attachment, US Letter, 0.5" hanging indent, TNR 12pt).
  ...
  - `src/lib/library/goodreads-csv.ts` — Goodreads export ISBN cell parse + `matchGoodreadsRatings` (ISBN-only; skip existing by default); unit tests `__tests__/goodreads-csv.test.ts`.
  - `src/lib/library/server/goodreads-import.ts` — parse Goodreads library CSV + preview/apply ratings (owner settings `/settings/library/goodreads`).
  - `src/lib/library/server/book-actions.ts` — `createBookAction`, `updateBookAction`, `softDeleteBookAction`, `undoSoftDeleteBookAction`, `reviewSaveAction`, `undoReviewSaveAction` ([071](docs/decisions/071-review-queue-authorless-undo.md)), `markNeedsShelfAction` ([074](docs/decisions/074-review-tab-corrections.md)), `createPersonAction`, `updateReadingStatusAction`, `updateBookPersonalFieldsAction` (owner-only rating + personal_notes + **owned** from book detail — [103](docs/decisions/103-library-not-owned-session-1.md)). Returns `{ kind, success?, message?, bookId?|personId? }`.
  ...
  - `src/lib/library/server/series-settings-actions.ts` / `series-settings-book-counts.ts` — `/settings/library/series` CRUD + guarded soft-delete; `include_in_citation` toggle ([142](docs/decisions/142-series-include-in-citation.md)).
  - `src/lib/library/turabian/publication.ts` — `formatSeriesSegment` respects `series_include_in_citation` (false → omit segment; catalog series still hydrated — [142](docs/decisions/142-series-include-in-citation.md)).
...
- **Projects helpers** at `src/lib/projects/` (schema: [docs/POS_Schema_v1.md](docs/POS_Schema_v1.md#projects), migrations `20260603170000_ppp_projects_v1.sql`, `20260603200000_projects_add_not_started_lifecycle.sql`, `20260604030000_ppp_project_tasks_myn.sql`, `20260604100000_project_updates_progress.sql`, `20260709164016_projects_email_inbox_and_domain_colors.sql`, `20260723170000_profiles_task_prefs.sql`; MYN design: [docs/MYN_TASKS_DESIGN.md](docs/MYN_TASKS_DESIGN.md)):
  ...
  - `src/lib/projects/task-views.ts` — saved-view parse/resolve + soft-cap truncate ([128](docs/decisions/128-myn-fall-polish.md)); unit tests `__tests__/task-views.test.ts`.
  - `src/lib/projects/now-task-rail.ts` — desktop Now rail JSON path + `taskFormAction` / payload guard ([205](docs/decisions/205-desktop-now-task-rail.md)); unit tests `__tests__/now-task-rail.test.ts`.
  - `src/lib/projects/server/loaders.ts` — tree, week updates, carry-forward, `loadLatestHealth`.
  - `src/lib/projects/server/task-loaders.ts` — `loadTasks` (zoned MYN + FRESH + 50 soft-cap), `loadDashboardNowTasks` (Critical/Opportunity counts for dashboard mobile tile), `loadNowTaskRail` (desktop Now rail JSON — [205](docs/decisions/205-desktop-now-task-rail.md)), `loadWeekTasks` (starting_this_week + carried_over — [165](docs/decisions/165-mcp-list-week-tasks.md)/[184](docs/decisions/184-mcp-monday-protocol-finetune.md)), `loadOpenTaskIdsByAssignmentId`, `loadTaskSeriesByIds`, `loadLinksByProject`.
  - `src/lib/projects/week-tasks.ts` — `clampWeekTaskDays` / `compareWeekHorizonTasks` / `summarizeWeekTasksByProject` / `isCarriedOverPriority` for MCP `list_week_tasks` ([165](docs/decisions/165-mcp-list-week-tasks.md), [184](docs/decisions/184-mcp-monday-protocol-finetune.md)); unit tests `__tests__/week-tasks.test.ts`.
  - `src/lib/projects/deferred.ts` — `isProjectDeferred` (`deferred_until > today` — [184](docs/decisions/184-mcp-monday-protocol-finetune.md)).
  ...
  - `src/lib/projects/server/task-prefs-actions.ts` — profile default project + saved views CRUD ([128](docs/decisions/128-myn-fall-polish.md)).
  - `/projects` — tree + metadata sheet (links in edit mode) + domain color picker; `depends('app:projects:tree')`.
  - `/tasks` — MYN task page (legacy `/projects/tasks` 308-redirects here); `depends('app:projects:tasks')`; Chicago today via `ymdInChicago()`; `?view=` / `?all=1` ([128](docs/decisions/128-myn-fall-polish.md)). Desktop Now pane: `GET /tasks/now.json` + `<DesktopTaskRail>` right of main ([205](docs/decisions/205-desktop-now-task-rail.md)/[206](docs/decisions/206-outlook-right-now-pane.md)).
  - `/dashboard` — desktop two-column home (status + modules + sermons left; Due soon + Due to meet right); **Now tasks live in the persistent right pane** (`md+`, collapsible, hidden on `/tasks` — [206](docs/decisions/206-outlook-right-now-pane.md)); mobile glance without task list ([132](docs/decisions/132-desktop-home-dashboard.md)).
...
- **Sermons helpers** at `src/lib/sermons/` (schema migrations `20260717190000_ppp_sermons_v1.sql`, `20260902152000_sermon_venues_context_type.sql`; Session 0 [090](docs/decisions/090-sermons-session-0.md), Session 1 [091](docs/decisions/091-sermons-session-1.md), Session 2 [095](docs/decisions/095-sermons-by-book-stats.md), venue type [217](docs/decisions/217-sermon-venues-context-type.md)):
  - `src/lib/types/sermons.ts` — `CONTEXT_TYPES`, list/venue/passage view-models (`SermonVenueRow.context_type` default), `DashboardSermonRow`, `ByBookRow` / `ByBookListFilters` / `ByBookSummary`.
 ...
 - `src/lib/sermons/venue-context.ts` — `parseContextType` / `contextTypeForVenue` (venue type → sermon context auto-fill — [217](docs/decisions/217-sermon-venues-context-type.md)); unit tests `__tests__/venue-context.test.ts`.
...
- **Sports helpers** at `src/lib/sports/` (schema migration `20260902221000_ppp_sports_v1.sql`; Session 1 [218](docs/decisions/218-sports-session-1.md)):
  ...
  - CLI / GHA: `npm run sports:sync` (`--standings` for teams+records); `.github/workflows/sports-sync.yml` every 10m (`npx tsx`, no `npm ci` — script must not import `dotenv` or other packages — [227](docs/decisions/227-sports-sync-gha-dotenv.md), scoreboard one day at a time [228](docs/decisions/228-sports-espn-scoreboard-date-range.md), CFB beyond FBS [221](docs/decisions/221-sports-cfb-beyond-fbs.md)).
...
- **Classwork helpers** at `src/lib/classwork/` (schema migrations `20260724220000_ppp_classwork_v1.sql`, `20260801120600_ppp_classwork_papers_v1.sql`, `20260831220000_classwork_canvas_ids.sql`; Session 0 [150](docs/decisions/150-classwork-session-0.md), Session 1 [153](docs/decisions/153-classwork-session-1.md), Session 2 [161](docs/decisions/161-classwork-session-2.md), Papers 0–1 [188](docs/decisions/188-classwork-research-papers-session-0.md)/[189](docs/decisions/189-classwork-papers-session-1.md), Canvas import [216](docs/decisions/216-canvas-classwork-import.md)):
 ...
 - `src/lib/classwork/canvas-map.ts` — Canvas → classwork mapping (Chicago due dates, Covenant name parse, kind heuristic, skip rules); unit tests `__tests__/canvas-map.test.ts` ([216](docs/decisions/216-canvas-classwork-import.md)).
 - `src/lib/classwork/canvas-client.ts` / `server/canvas-import.ts` — paginated Canvas REST + plan/apply; CLI `scripts/classwork-canvas-import.ts` (`npm run classwork:canvas-import`, `--apply` to write). Token env-only.
 - `src/lib/classwork/paper-sources.ts` — client-safe `PaperSourceView` (book XOR essay, citation inputs hydrated server-side) + `paperSourceTitle` / `paperSourceBookId`; `PaperGroupView` + `groupPaperSources` bucket helper (Ungrouped first; stale `group_id` falls back to Ungrouped — [190](docs/decisions/190-classwork-papers-session-2.md)); unit tests `__tests__/paper-sources.test.ts`.
 - `src/lib/classwork/paper-bibliography.ts` — `formatPaperBibliographyEntries` / `compilePaperBibliography`: flat Turabian merge of book + essay entries (shared sort key; unsigned essays dropped) — do **not** widen library's books-only `formatCompiledBibliography` ([189](docs/decisions/189-classwork-papers-session-1.md)); unit tests `__tests__/paper-bibliography.test.ts`.
 ...
 - `src/lib/classwork/server/paper-loaders.ts` — `loadPapers` (+ source counts), `loadPaperDetail`, `loadPaperGroups`, `loadPaperSourceViews` (essays by id + one `loadBookDetail` per distinct book, parallel — documented round-trip exception [189](docs/decisions/189-classwork-papers-session-1.md)).
 - `src/lib/classwork/server/paper-actions.ts` — paper CRUD (P1 **stamp + lock**: assignment link re-stamps `course_id`/`due_date` on every save), `openResearchPaperAction` (create-or-open 1:1), source attach (revive-by-PK per footgun NEW-D) / notes / remove, `createNotOwnedSourceAction` (free-form `owned=false` stub + attach; dedupes by normalized title); research-group actions (`createPaperGroupAction`, `renamePaperGroupAction`, `reorderPaperGroupsAction` ordered-id rewrite, `softDeletePaperGroupAction` **G1: nulls source `group_id` first**, `setPaperSourceGroupAction` same-paper guard — [190](docs/decisions/190-classwork-papers-session-2.md)).
...
- **Contacts helpers** at `src/lib/contacts/` (schema migrations `20260725020000_ppp_contacts_v1.sql`, `20260725180115_contacts_touch_kind_and_cadence_ui.sql`, `20260824190000_contacts_period_cadence_v1.sql`; Sessions 0–3 + [210](docs/decisions/210-contacts-semester-period-cadence.md) + due integrity [212](docs/decisions/212-contacts-due-integrity.md)):
  - `src/lib/types/contacts.ts` — `CONTACT_FREQUENCIES`, **labels** Quarterly / Semester / Annual (enum `semiannual` stays) + `CONTACT_FREQUENCY_SHORT_LABELS` for compact roster, grades, list kinds, due/pace view-models (legacy `DEFAULT_CONTACT_CADENCE_DAYS` / `ContactDueRow` / `ContactSearchHit`, `ListMembershipMaps`).
...
- **`sports:sync`** — ESPN → `sports_games` / optional teams+standings (`--standings`). Needs `PUBLIC_SUPABASE_URL` + `SUPABASE_SERVICE_ROLE_KEY`. Also runs from GitHub Actions every 10m ([220](docs/decisions/220-sports-sync-github-actions.md)). GHA is `npx tsx` with no `npm ci` — `scripts/sports-sync.ts` must stay package-free ([227](docs/decisions/227-sports-sync-gha-dotenv.md)). Scoreboard `dates=` is one YYYYMMDD per request ([228](docs/decisions/228-sports-espn-scoreboard-date-range.md)). CFB catalog is paginated (not `limit=500`); scoreboards cover FBS/FCS/D2/D3/NAIA ([221](docs/decisions/221-sports-cfb-beyond-fbs.md)).
...
| `.env.local` | Prod secrets and public client config | `PUBLIC_SUPABASE_URL`, `PUBLIC_SUPABASE_ANON_KEY`, `RESEND_API_KEY`, **`ANTHROPIC_API_KEY`** (optional — mirror of Supabase Edge secret for `supabase functions serve` / local OCR dev only), **`LIBRARY_SRC_DATABASE_URL`**, **`LIBRARY_DST_DATABASE_URL`**, **`LIBRARY_MIGRATE_CONFIRM`** (Postgres URIs, typically both from Supabase Dashboard **Connect → Direct** — see [`scripts/library-migrate-local-to-prod/README.md`](scripts/library-migrate-local-to-prod/README.md)), **`SUPABASE_SERVICE_ROLE_KEY`** + **`POS_OWNER_ID`** (CLI / ppp MCP / library-import — never browser; [144](docs/decisions/144-ppp-mcp-readonly-v1.md)), **`CANVAS_HOST`** + **`CANVAS_TOKEN`** (CLI classwork import only — never browser; [216](docs/decisions/216-canvas-classwork-import.md)) |
