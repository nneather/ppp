...
- **Library helpers** at `src/lib/library/`:
  ...
  - `src/lib/library/ocr-invoke-client.ts` — `invokeOcrScriptureRefs` + Edge error parsing (`readEdgeErrorBody`); batch form calls Edge from browser to avoid Vercel timeout on long PDF OCR ([030](docs/decisions/030-ocr-pdf-input.md)).
