...
- **Projects helpers** at `src/lib/projects/` (schema: [docs/POS_Schema_v1.md](docs/POS_Schema_v1.md#projects), migrations `20260603170000_ppp_projects_v1.sql`, `20260603200000_projects_add_not_started_lifecycle.sql`, `20260604030000_ppp_project_tasks_myn.sql`, `20260604100000_project_updates_progress.sql`, `20260709164016_projects_email_inbox_and_domain_colors.sql`, `20260723170000_profiles_task_prefs.sql`; MYN design: [docs/MYN_TASKS_DESIGN.md](docs/MYN_TASKS_DESIGN.md)):
  ...
  - `src/lib/projects/server/task-loaders.ts` — `loadTasks` (zoned MYN + FRESH + 50 soft-cap), `loadDashboardNowTasks` (Critical/Opportunity only for desktop home — [132](docs/decisions/132-desktop-home-dashboard.md)), `loadWeekTasks` (coming-week horizon all zones — [165](docs/decisions/165-mcp-list-week-tasks.md)), `loadTaskSeriesByIds`, `loadLinksByProject`.
  - `src/lib/projects/week-tasks.ts` — `clampWeekTaskDays` / `compareWeekHorizonTasks` / `summarizeWeekTasksByProject` for MCP `list_week_tasks` ([165](docs/decisions/165-mcp-list-week-tasks.md)); unit tests `__tests__/week-tasks.test.ts`.
...
- **Contacts helpers** at `src/lib/contacts/` (schema migrations `20260725020000_ppp_contacts_v1.sql`, `20260725180115_contacts_touch_kind_and_cadence_ui.sql`; Session 0 [175](docs/decisions/175-contacts-session-0.md), Session 1 [178](docs/decisions/178-contacts-session-1.md), Session 2 [180](docs/decisions/180-contacts-session-2.md), Session 3 [182](docs/decisions/182-contacts-session-3-lists-cadence-touch-kinds.md)):
  - `src/lib/types/contacts.ts` — `CONTACT_STATUSES`, `CONTACT_TOUCH_KINDS` (meet\|card), list/filter view-models, `DEFAULT_CONTACT_CADENCE_DAYS` (90), `ContactDueRow` / `ContactSearchHit`.
  ...
  - `src/lib/contacts/server/loaders.ts` — `loadContacts`, `loadContactsDue`, `searchContacts`, `loadHouseholds`, `loadContactLists`, `loadContactListMembers` (C2 filter), `parseContactsListFilters`; last-touch queries filter `kind=meet`.
  - `src/lib/contacts/server/actions.ts` — contact/household/list CRUD; meet Log Contact / Log all; `logListCardsAction` (`kind=card`); `updateContactCadenceDefaultAction`; list-member add (revive soft-deleted by PK).
  - Routes: `/contacts` (tabs Contacts \| Households \| Lists + Sheets + Log Contact / Log cards); `/settings/contacts/lists` 308→ Lists tab; dashboard Due to meet. Desktop sidebar only (mobile tab bar stays at 5).
