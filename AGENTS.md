...
- **Library helpers** at `src/lib/library/`:
  ...
  - `src/lib/library/ocr-invoke-client.ts` — `getPdfPageCount`, `invokeOcrScriptureRefs` (+ optional `pdf_page_index`), Edge error parsing; batch form uploads PDF once then OCRs **one page per Edge call** to stay under Supabase idle limit ([030](docs/decisions/030-ocr-pdf-input.md)).
