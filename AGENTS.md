...
- **`mcp:smoke` / [`scripts/ppp-mcp/`](scripts/ppp-mcp/)** — local stdio **read-only** MCP for Cursor + Claude Code ([144](docs/decisions/144-ppp-mcp-readonly-v1.md), week horizon [165](docs/decisions/165-mcp-list-week-tasks.md)). Loads `.env` + `.env.local`; requires `PUBLIC_SUPABASE_URL`, `SUPABASE_SERVICE_ROLE_KEY`, `POS_OWNER_ID` (owner assert). Tools wrap existing loaders. Wiring: [`scripts/ppp-mcp/README.md`](scripts/ppp-mcp/README.md). Helpers: [`src/lib/mcp/bible-book.ts`](src/lib/mcp/bible-book.ts), [`course.ts`](src/lib/mcp/course.ts).
