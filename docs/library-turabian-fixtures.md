...
- **Series enumeration:** do **not** emit `Vol. N.` for commentary-in-series (e.g. Interpretation / Achtemeier). Append the bare series number after the series title (`… Preaching 27.`, `ESV Expository Commentary 3.`). Keep `Vol. N.` for multi-volume works (Keener) and `N vols.` for multi-vol sets in a series (Zimmerli / Lechler — no bare series number from `volume_number`). See [113](decisions/113-commentary-series-volume-omit.md).
