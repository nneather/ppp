...
- **ISBN scan UX (follow-up)** — [`isbn.ts`](../../src/lib/library/isbn.ts): `parseIsbnWithChecksum` / ISBN-10 and ISBN-13 check digits. [`/library/add`](../../src/routes/library/add/+page.svelte): double-decode confirmation window, tap overlay on video, optional one-shot auto-start after `videoEl` binds, camera tips + `permissions.query` when available, checksum on manual entry. [`scan-session.ts`](../../src/lib/library/scan-session.ts): `library_scan_session_v1` flag for **Save & scan another** + **Back to scanner** on [`/library/books/new`](../../src/routes/library/books/new/+page.svelte); mobile scan layout on `<BookForm>` (`scanSessionLayout` + collapsible middle columns + imported summary).
...
- **ISBN validation at OL boundary:** `fetchOpenLibraryPrefill` requires `parseIsbnWithChecksum` so bogus length-13 scans never hit Open Library. Web apps **cannot** force the OS/browser to “remember” camera forever; persistence is site permission + HTTPS + not private-browsing; in-app copy + optional `navigator.permissions.query({ name: 'camera' })` when supported.
- **Scan session loop:** `sessionStorage` key `library_scan_session_v1` set when leaving `/library/add` for `/library/books/new` (any path through `applyOpenLibraryAndGo`); consumed on new-book mount. Enables **Save & scan another** (secondary submit) and **Back to scanner** without duplicating form state server-side.
...
- `open-library-prefill.ts` — re-exports `normalizeIsbnDigits` from [`isbn.ts`](../../src/lib/library/isbn.ts); `fetchOpenLibraryPrefill` requires check-digit validity; edition + optional work + bounded author fetches; `publisher_location`, `edition`, `genreSuggested` on `OpenLibraryBookPrefill`; [`isbn.ts`](../../src/lib/library/isbn.ts) / [`scan-session.ts`](../../src/lib/library/scan-session.ts); `book-copy-text.ts` — small pure helpers; not registered in `components.mdc` (no new shared UI primitive).
