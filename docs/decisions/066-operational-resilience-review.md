...
- Headline verdict (as of this review): **backups are live; restore is unproven and the smoke script is guaranteed to fail as written.** Run 28830982000 (2026-07-06 23:43 UTC) uploaded `2026/ppp-invoicing-2026-07.dump` (32.7 KiB) + `2026/ppp-library-2026-07.dump` (385.5 KiB) to R2; all five secrets set same day. But `restore-smoke.sh` line 75 uses `pg_dump --where` (does not exist in PostgreSQL 17), and the schema-only `profiles` load trips on `auth.users` FK / triggers / RLS under `ON_ERROR_STOP=1`. Never executed.
- **Resolved in [079](079-ops-hardening-backups-restore-revoke.md)** (2026-07-09): weekly cron + profiles/projects dumps; restore-smoke rewritten and run green; denorm REVOKE (PUBLIC + anon). Nav watchdog was already in [072](072-pwa-cold-start-resilience.md).
...
All five landed in ops hardening — Q14 via [072](072-pwa-cold-start-resilience.md); Q10–Q13 via [079](079-ops-hardening-backups-restore-revoke.md).

## Schema changes

- None this session. Q13 REVOKE shipped in [079](079-ops-hardening-backups-restore-revoke.md).
...
- Scheduled-run failure visibility: GitHub only emails the workflow-file author on cron failures — first unattended **weekly** fire after 079; check the Actions tab after.
- Quarterly restore re-run cadence — library restore smoke proven in 079; revisit cadence later if desired.
