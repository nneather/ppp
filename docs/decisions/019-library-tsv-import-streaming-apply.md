...
- **Import prep** — [`prepareLibraryBooksImport`](src/lib/library/server/books-csv.ts) with [`sniffLibraryImportFormat`](src/lib/library/server/books-csv.ts): `.tsv` forces TSV; otherwise first header line: more commas than tabs → CSV, else TSV (so a tab-separated file is not parsed as CSV when renamed to `.csv`).
