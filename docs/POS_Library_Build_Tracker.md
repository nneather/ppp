...
_Last updated: July 6, 2026 | Module: Library (2nd) | **Trip build: complete** | **Wave 2: Session 2 complete — Session 3 next**_
...
| Task | Done | Notes |
|------|:----:|-------|
| Citation generation module at `src/lib/library/turabian/` — pure TS, no DB calls inside formatters. Input: hydrated book + citation kind. Output: formatted string. | ☑ | `npm run test` (9 unit cases). See **031**. |
| Source-type handlers — single-author book, edited volume, multi-volume work, commentary in series, standalone commentary, reference work (edited), reference work (single-author), Bible | ☑ | `resolveCitationSourceType` + `format.ts`. |
| Edge cases — edition, reprint (`original_year` + `reprint_*`), translator (from `book_authors` role='translator' — structured now), multi-volume with `volume_number`, abbreviated series in footnotes | ☑ | Reprint + translator + series in `publication.ts` / `names.ts`. |
| Author formatting by count — 1 / 2 / 3+ author rules per Turabian, sort by `book_authors.sort_order`. Footnote vs bibliography differ for 3+ authors. | ☑ | `names.ts` (`et al.` threshold 4 in footnotes). |
| Citation UI on book detail — Copy Footnote / Copy Full Footnote / Copy Bibliography buttons replace Session 6's raw-field copy buttons (or coexist; raw-field stays useful for non-academic copy paths) | ☑ | Coexist with raw-field copy; HTML+plain clipboard. |
| Bibliography builder — select N books from `/library` list, export sorted bibliography | ☑ | `/library/bibliography?ids=` + bulk bar on `/library`. HTML+plain clipboard (Q4). |
| Citation QA pass — generate footnote + bibliography for 20 spot-check books across every source type, verify against Turabian skill | ☐ | Unit QA shipped; full 20-row shelf pass deferred August per trip policy. |
