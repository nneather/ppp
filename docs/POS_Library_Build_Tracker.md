...
- [ ] Owner can list → view → create → edit → soft-delete → undo a book end-to-end on desktop and phone — _pending hands-on smoke test_.
- [ ] Add a new book with 2 authors (different roles), 3 categories (1 primary), series + volume → 2 `book_authors` rows, 3 `book_categories` rows, `series_id` + `volume_number` populated — _pending hands-on smoke test_.
- [ ] Edit flow: remove 1 author, add 1 editor, reorder remaining → junction rows reflect exactly the new state; audit log shows 1 DELETE + 1 INSERT + N UPDATEs — _pending hands-on smoke test; the audit-log UI at `/settings/audit-log` filtered to Library should show every junction change attributed to the parent book per the new `record_id = book_id` resolution_.
- [ ] B14 canonicalization combobox prompt fires when creating a person whose last_name + first_initial match an existing person — _pending hands-on smoke test (inline yellow-caption hint, not full combobox; Session 5 builds the real `<CanonicalizingCombobox>`)_.
- [x] No hand-written DB types in `src/`. Grep for `type Book = {` returns zero hits.
- [x] `npm run check` passes
- [ ] If a migration was added: `npm run supabase:gen-types` ran and `src/lib/types/database.ts` is in the same commit — **PENDING**: run before next commit so the post-delta-v1 column shapes land in types and the `// TODO post-delta-v1` compat shims in `src/lib/library/server/loaders.ts` + `book-actions.ts` can be reverted.
- [x] `docs/decisions/NNN-<slug>.md` filed using the `AGENTS.md` template (`002-library-session-0-audit.md` + `003-library-books-vertical-slice.md`).
...
| `<ScriptureReferenceForm>` on book detail — wraps `<SourcePicker>` (auto-populated to current book); fields: bible_book, chapter_start, verse_start, chapter_end, verse_end, page_start, page_end, needs_review, review_note, source_image_url (manual upload, no OCR) | ◐ | Component built Session 1.5 / Track E (`src/lib/components/scripture-reference-form.svelte`); **NOT yet wired into `/library/books/[id]`** — that's the remaining Session 2 integration work. Manual `source_image_url` text input only; upload UI deferred until Storage bucket name is decided (Open Question 3). |
| Verify `compute_verse_abs` trigger fires on INSERT and on UPDATE of any chapter/verse column | ◐ | Implicitly verified by `library_scripture_fixture.sql` loading 20 rows and `search_scripture_refs(...)` returning correctly-bounded results. Explicit SQL assertion test still pending — paste-ready in the Session 2 build. |
...
| Image upload to Supabase Storage private bucket for `source_image_url` | ☐ | Signed URL on read with 1h TTL per S8. **Manual upload only** — automated parsing is post-trip. Bucket name pending Open Question 3. |

**Acceptance:**
- [x] `<SourcePicker>` component exists and is documented in `src/lib/library/polymorphic.ts` with the four-table reuse comment.
- [ ] `compute_verse_abs` trigger verified on both INSERT and UPDATE paths — _implicit pass via fixture data; explicit SQL test pending Session 2 wiring_.
- [x] `search_scripture_refs()` deployed; returns expected rows for verse-level, chapter-level, and whole-book search cases.
- [ ] Owner can create a scripture_reference on a book detail page with optional image upload — _pending Session 2 wiring of `<ScriptureReferenceForm>` into the book detail page_.
...
- [x] `npm run check` passes
- [ ] If a migration was added: `npm run supabase:gen-types` ran and `src/lib/types/database.ts` is in the same commit — **PENDING** (rolls up with Session 1's gen-types step).
- [x] `docs/decisions/NNN-<slug>.md` filed using the `AGENTS.md` template (`003-library-books-vertical-slice.md` Session 2 jump-start section).
- [ ] viewer can INSERT scripture_references and UPDATE their own rows; viewer cannot UPDATE the owner's rows; viewer cannot DELETE — _deferred: solo use until a collaborator joins_.
...
| Task | Done | Notes |
|------|:----:|-------|
| `/settings/library/people` — list, search by last name, edit, **merge (owner-only)** | ☐ | Merge: pick canonical + merged-away → re-point `book_authors` → soft-delete merged-away person. Audit log: non-revertible. Reuses canonicalization combobox pattern. |
...
| 3 | Image upload max size / client-side compression before Supabase Storage? | ☐ Open — resolve at Session 2 |
