...
**Last updated:** 2026-07-21 — **Resend inbound secrets set** ([098](docs/decisions/098-resend-inbound-webhook-secrets.md)): `RESEND_WEBHOOK_SECRET` + `INBOUND_TASK_*` refreshed on prod from live Resend webhook. Prior: Vercel CI build gate ([097](docs/decisions/097-vercel-deploy-ci-build-gate.md)).
...
| Projects | [docs/POS_Projects_Build_Tracker.md](docs/POS_Projects_Build_Tracker.md) | ✅ **v1 complete** + **email-to-task + domain colors** ([077](docs/decisions/077-email-to-task-and-domain-colors.md)) + project select scroll ([080](docs/decisions/080-tasks-project-select-scroll.md)) + domain-root task options ([081](docs/decisions/081-tasks-project-options-domains.md)). Tree/check-in, dashboard/filters, MYN `/tasks` (top-level nav; `/projects/tasks` redirects), links, audit. **Inbound secrets set** ([098](docs/decisions/098-resend-inbound-webhook-secrets.md)) — owner smoke: forward → Email Inbox. **Viewer access:** owner-only by design. **Backlog:** polish, global Now ([MYN_TASKS_DESIGN.md](docs/MYN_TASKS_DESIGN.md)). |
...
- [097 — Vercel deploy CI build gate](docs/decisions/097-vercel-deploy-ci-build-gate.md) (2026-07-21) — duplicate `Label` broke Vercel; fixed on main in 094; CI adds `npm run build`; recommend required checks on `main`.
...
**Repo gate:** `npm run build` **exit 0** locally (2026-07-21, [097](docs/decisions/097-vercel-deploy-ci-build-gate.md)); HEAD `a31815f` Vercel Production **success**. CI workflow now includes build (lands with this session’s commit).
...
4. **MYN adoption trial** ([064](docs/decisions/064-usage-retrospective-review.md) Q1) — `/tasks` as the only task list through ~2026-07-20, then re-decide (adopted vs. freeze + cancel global-Now). **Email capture secrets set** ([098](docs/decisions/098-resend-inbound-webhook-secrets.md)) — smoke: forward from allowlisted mailbox to `tasks@zeneoldai.resend.app`.
