...
**Sports:** `/sports` scoreboard + follow stars + owner Refresh; dashboard glance; GHA `.github/workflows/sports-sync.yml` every 10m (`npm run sports:sync`); CFB catalog is FBS–NAIA ([221](docs/decisions/221-sports-cfb-beyond-fbs.md)); Vercel daily cron is Hobby backup only. Helpers `src/lib/sports/`; migration `20260902221000_ppp_sports_v1.sql` ([218](docs/decisions/218-sports-session-1.md), [220](docs/decisions/220-sports-sync-github-actions.md)).
