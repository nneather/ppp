...
**Last updated:** 2026-09-11 — Sports GHA sync `dotenv` crash ([227](docs/decisions/227-sports-sync-gha-dotenv.md)); Contacts couple last-name backfill ([226](docs/decisions/226-contacts-couple-last-name-backfill.md)).
...
| Sports | [docs/POS_Sports_Build_Tracker.md](docs/POS_Sports_Build_Tracker.md) | ✅ Session 1 scoreboard + **GHA ESPN sync** every 10m ([220](docs/decisions/220-sports-sync-github-actions.md)); GHA script is package-free so `npx tsx` works ([227](docs/decisions/227-sports-sync-gha-dotenv.md)); CFB follow list is FBS–NAIA ([221](docs/decisions/221-sports-cfb-beyond-fbs.md)); standings are **division tables** with real W-L ([224](docs/decisions/224-sports-standings-divisions.md)). Hobby cron is backup only. |
...
**Repo gate:** Sports GHA dotenv crash [227](docs/decisions/227-sports-sync-gha-dotenv.md) — `npm run check` **0 errors / 0 warnings**; sports unit tests 19/19 (2026-09-11). Needs a push to `main` before the next 10m cron.
