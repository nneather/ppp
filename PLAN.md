...
**Last updated:** 2026-07-23 — B&H free-text → `B&H Academic` ([131](docs/decisions/131-bh-publisher-link-backfill.md)); owner glance done; Email Inbox → Personal ([130](docs/decisions/130-retire-email-inbox-default-personal.md)).
...
- `/dashboard` — desktop two-column home (status + modules + upcoming sermons; sticky Critical/Opportunity Now); mobile glance without task list ([131](docs/decisions/132-desktop-home-dashboard.md)).
...
**Sermons:** `/sermons` list + Sheet; `/sermons/by-book` commentary × sermon spine; `/settings/sermons/venues`; helpers `src/lib/sermons/` (incl. `loadUpcomingSermons`); migration `20260717190000_ppp_sermons_v1.sql` ([091](docs/decisions/091-sermons-session-1.md), [095](docs/decisions/095-sermons-by-book-stats.md), [131](docs/decisions/132-desktop-home-dashboard.md)).
...
**Repo gate:** `npm run check` + `npm run test` **2026-07-23** ([131](docs/decisions/132-desktop-home-dashboard.md); **0 errors**, 299 tests). Prior: [129](docs/decisions/129-form-control-height-consistency.md).
