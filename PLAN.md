...
**Last updated:** 2026-09-17 — Contacts roster current check ([230](docs/decisions/230-contacts-roster-current-check.md)).
...
- [230 — Contacts roster current check](docs/decisions/230-contacts-roster-current-check.md) (2026-09-17) — Green check beside names whose Q/S/A period is already met or skipped.
...
**Sports:** `/sports` scoreboard + follow stars + owner Refresh; dashboard glance; GHA `.github/workflows/sports-sync.yml` every 10m (`npx tsx` — no `npm ci`, script must not import npm packages — [227](docs/decisions/227-sports-sync-gha-dotenv.md)); scoreboard `dates=` is one YYYYMMDD per request ([228](docs/decisions/228-sports-espn-scoreboard-date-range.md)); CFB catalog is FBS–NAIA ([221](docs/decisions/221-sports-cfb-beyond-fbs.md)); standings are division tables ([224](docs/decisions/224-sports-standings-divisions.md)); Vercel daily cron is Hobby backup only. Helpers `src/lib/sports/`; migration `20260902221000_ppp_sports_v1.sql` ([218](docs/decisions/218-sports-session-1.md), [220](docs/decisions/220-sports-sync-github-actions.md)).

**Sermons:** `/sermons` list + Sheet (venue type auto-fills context — [217](docs/decisions/217-sermon-venues-context-type.md)); `/sermons/by-book` commentary × sermon spine (series label + multi-part collapse + latest sermon date — [134](docs/decisions/134-sermons-by-book-series-dedupe.md)); `/settings/sermons/venues`; helpers `src/lib/sermons/` (incl. `loadUpcomingSermons`, `venue-context.ts`); migrations `20260717190000_ppp_sermons_v1.sql`, `20260902152000_sermon_venues_context_type.sql` ([091](docs/decisions/091-sermons-session-1.md), [095](docs/decisions/095-sermons-by-book-stats.md), [132](docs/decisions/132-desktop-home-dashboard.md), [217](docs/decisions/217-sermon-venues-context-type.md)).
...
**Repo gate:** Contacts roster current check [230](docs/decisions/230-contacts-roster-current-check.md) — `npm run check` **0 errors / 0 warnings**; `npm run test` **554/554** (2026-09-17).
...
- **Contacts Christmas Has-address seed** ([211](docs/decisions/211-contacts-module-review.md)) — November; list exists, import did not fill it.
